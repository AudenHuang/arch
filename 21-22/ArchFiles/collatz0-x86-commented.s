...
.LBB1_2:
	...
	movl	%edx, (%rax,%rcx,4)	# index array_length (%rcx) of the array (%rax) is set to be current (%edx) (address at %rax+%rcx*4) (line 14 in collatz.c)
	...
	jne	.LBB1_4					# Jumps to .LBB1_4 if current is not equal to 1 (by checking the flags)
...
.LBB1_4:
	...
	jne	.LBB1_6					# Jumps to .LBB1_6 if current%div is not equal to 0 (if zero flag is not set to 1)
	movl	-8(%rbp), %eax		# %eax = current
	...
	callq	collatz_recurse		# Calls the collatz_recurse function (with the %edi above)
	andb	$1, %al				# Performs bitwise and operation on %al with 1, clearing all bits to 0 except for the least significant bit, which may be 1 (true) or 0 (false)
	movb	%al, -1(%rbp)		# Copies value of %al to -1 byte below base pointer (the return boolean)
	jmp	.LBB1_9					# Jumps to .LBB1_9
.LBB1_6:
	movl	-8(%rbp), %eax		# %eax = current
	movl	%eax, -12(%rbp)		# Stores current at -12 bytes below the base pointer in the stack 
	...
	jle	.LBB1_8					# Jumps to .LBB1_8 if current is less or equal to (INT_MAX - add)/mult (by checkng the flags)
	movb	$0, -1(%rbp)		# Copies "0" (false) to -1 byte below the base pointer (the boolean to be returned by the function)
	jmp	.LBB1_9					# Jumps to .LBB1_9
.LBB1_8:
...
.LBB1_9:
	...
	popq	%rbp				# Pop the base pointer from the previous stack frame back into %rbp
	.cfi_def_cfa %rsp, 8
	retq						# Return the function (pop the return address from stack to %rip)
.Lfunc_end1:
