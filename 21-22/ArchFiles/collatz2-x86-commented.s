...
collatz:
	...
	jle	.LBB0_8						# Jumps to .LBB0_8 if start is equal or less than 0 (by checking the ZF)
...
.LBB0_3:
	...
	je	.LBB0_10					# Jumps to .LBB0_10 if array_length equals to MAXARRAYSIZE (by checking the flags)
.LBB0_4:
	movl	%edi, (%r9,%rbx,4)  	# Index array_length (%rbx used to hold long-lived values that should be preserved across calls) of the array (%r9 = int_array) is set to be current (%edi = start) (address at %r9 +%rbx*4)(line 14 in collatz.c)
	...
	je	.LBB0_2						# Jumps to .LBB0_2 if current%div == 0 (by checkingthe ZF)
	...
	jl	.LBB0_11					# Jumps to .LBB0_11 if current is less than (INT_MAX-add)/mult (by checking the flags)
...
.LBB0_11:
	addl	$1, %ebx				# array_length = array_length + 1
.LBB0_12:
	xorl	%eax, %eax				# Exclusive or bitwise operation performed on return register with itself, which clears register to 0s and represents false (makes result = 0 thus false)
.LBB0_13:
	...
	.cfi_def_cfa %rsp, 8
	retq							# Return the function (pop the return address from stack, and return register %eax)
.Lfunc_end0:
